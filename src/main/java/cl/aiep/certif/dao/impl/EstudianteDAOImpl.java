...
public class EstudianteDAOImpl implements EstudianteDAO {
	...
	public List<EstudianteDTO> obtenerEstudiantes() {
		...
		for (Estudiante est : repository.findAll())
			estDTO.add(new EstudianteDTO(est.getRut(), est.getNombre(), est.getTelefono(), est.getDireccion(),
					est.getEmail(), est.getIdRegion(), est.getPassword(), est.getEnabled(), est.getIdCurso(),
					est.getEdad()));
		return estDTO;
	}
	...
	public EstudianteDTO obtenerEstudiante(String rut) {
		...
		if (optional.isPresent())
			est = optional.get();
		return new EstudianteDTO(est.getRut(), est.getNombre(), est.getTelefono(), est.getDireccion(), est.getEmail(),
				est.getIdRegion(), est.getPassword(), est.getEnabled(), est.getIdCurso(), est.getEdad());
	}
	...
	public boolean guardaEstudiante(EstudianteDTO est) {
		...
		try {
			...
			repository.save(new Estudiante(est.getRut(), est.getNombre(), est.getTelefono(), est.getDireccion(),
					est.getEmail(), est.getIdRegion(), UtilBCEncrypt.encripta(est.getPassword()), 1,
					cursRepo.getById(0), est.getEdad()));
	...
	public List<RegionDTO> obtieneRegiones() {
		...
		for (Region reg : regionrepo.findAll())
	...
	public CursoDTO obtieneCurso(String rut) {
		...
		CursoDTO retorno = new CursoDTO(cur.getId(), cur.getNombre(), cur.getImagen(), cur.getFecinicio(),
